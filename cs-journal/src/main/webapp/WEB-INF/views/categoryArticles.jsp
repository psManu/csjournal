...
<body>
	<p>category articles page</p>
	<c:if test="${not empty category}">
		<h3> <c:out value="${category.id}"></c:out> <c:out value="${category.name}"></c:out></h3>
		<br>
		<br>
		...
		<c:if test="${ not empty art_list }">
			<table>
				...
				<c:set var="url" value="/cs-journal/cat/${category.id}/${article.artId}/${article.artNameWithoutSpaces} "/>
